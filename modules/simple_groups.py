...
GROUP_KEYS = ["retail", "mid", "large"]
GROUP_NAMES = {"retail": "散戶", "mid": "中實戶", "large": "大戶"}


def group_labels(thresholds: tuple[int, int]) -> dict[str, str]:
    retail, large = thresholds
    return {
        "retail": f"散戶（{retail} 張以下）",
        "large": f"大戶（{large} 張以上）",
    }


def apply_custom_groups(frame: pd.DataFrame, thresholds: tuple[int, int]) -> pd.DataFrame:
    retail, large = thresholds
    if retail >= large:
        raise ValueError("散戶上限必須小於大戶下限。")
    if any(value not in BUCKET_LIMITS for value in thresholds):
        raise ValueError("門檻必須使用集保資料提供的級距邊界。")
    retail_cut = BUCKET_LIMITS.index(retail) + 1
    large_cut = BUCKET_LIMITS.index(large) + 1
    ranges = [BUCKET_COLUMNS[:retail_cut], BUCKET_COLUMNS[retail_cut:large_cut], BUCKET_COLUMNS[large_cut:]]
    result = frame.copy()
...
def build_stock_snapshot_average(chip: pd.DataFrame, xq: pd.DataFrame, current_date: pd.Timestamp, period: str, thresholds: tuple[int, int] = (50, 400), analysis_group: str = "large"):
